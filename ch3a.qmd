...
ef extract_translations(doc):
    ...
    for item in items:
        record_counter += 1                                                       # <3>
        record = {'id': record_counter}                                           # <4>
        spans = item.findall('span')                                              # <5>
        for span in spans:
            key = span.get('class')                                               # <6>
            if key == 'sn_pub':                                                   # <7>
                subs = span.findall('span')
                for sub in subs:
                    extract_key_value(record, sub)                                # <8>
            else:
...
2. extracts individual table cells with an XPath expression (explained above). For each
   HTML snippet...
3. increases the record counter by one
4. creates a  dictionary for the record structure, initially only with the identifier
5. all the important data elements are in `<span>` elements, so we have to find extract them
   (inside the current cell), and iterate over them
6. the semantic information is found in the class attribute, so we should save it
7. however there is a special span: `sn_pub` is a container of other spans, and we
   have to run a second iteration to extract its children
8. at the end we call the extract_key_value() method with the record structure and the
...
output_file = os.path.join(output_dir, 'authors.csv')                             # <6>
with open(output_file, 'w', encoding='utf-8') as csv_file:
    output_writer = csv.DictWriter(csv_file, fieldnames=column_names)
    output_writer.writeheader()
    output_writer.writerows(authors)

output_file = os.path.join(output_dir, 'translators.csv')                         # <7>
with open(output_file, 'w', encoding='utf-8') as csv_file:
    output_writer = csv.DictWriter(csv_file, fieldnames=column_names)
    output_writer.writeheader()
